...
apply_frame_on_frames <-
  function(rec, out, framed_by, i_start, nsamples)
  {
    is = i_start
    if (length(i_start) == 1)
...
truncate_to_same_length <- function(rec)
{
  ...
  return (
    apply_frame(
      rec,
      framed_by = "truncate_to_same_length",
      i_start = i_start,
      nsamples = min_n
    )
  )
...
frame_recordings <- function(rec,
                             framed_by = "frame_recordings",
                             i_start_diff = NULL,
                             nsamples = NULL,
                             ...
                             sound_velocity = 334)
{
  ...
  if (is.null(i_start_diff))
    i_start_diff = 1
  i_start = seq(i_start_diff,
                i_start_diff + nsamples - frame_size,
                frame_size / 2)
