...
set_source_list <- function(names,
                            ...
                            max_freq)
{
  return (lapply(1:length(names), function (i) {
    list(
      names = names[i],
      x_min = x_pos_min[i],
      x_max = x_pos_max[i],
...
set_frame_list <- function(names, t_start, t_end,f_min, f_max, sourceList)
{
  return (lapply(1:length(names), function (i) {
    list(
      name = names[i],
      t_start = t_start[i],
      t_end = t_end[i],
      sources = lapply(sourceList[[i]], function (s)
        s$name)
    )
  }))
}
...
apply_frames <- function(rec, frames)
{
  rec$frames = lapply(frames, function (frame) {
    is = rec$i_start + frame$t_start * rec$fs[1]
    nsamples = (frame$t_end - frame$t_start) * rec$fs[1]
    nsamples = min(min(rec$raw_nsamples - is + 1), nsamples)
    frame$i_start = is
    frame$nsamples = nsamples
    return (frame)
  })
  return (rec)
}
...
frame_to_signal_fft <- function(rec, all_frames)
{
  ...
  all_frames$signal_for_fft <-
    ...
    lapply(1:length(all_frames$t_start)
           , function (i)
           {
             ...
             lapply(rec$framed_raw_signal, function(s)
               s[i_start:i_end] * hn)
           })
  ...
  all_frames$fft <-
    lapply(1:length(all_frames$t_start), function (i)
...
optimize_offsets <-
  function(shared_peaks, receptors, sound_velocity)
  {
    sqr_sum_per_offset <- function(beta)
    {
      pos = optimize_all_time_lags_to_their_source_positions(
        lags = t(t(sp$shared_peaks$delays) - beta),
        receptors = receptors,
    ...
    beta_init = numeric(n_receptors - 1)
    opt_res = pracma::nelder_mead(sqr_sum_per_offset, beta_init)
    offsets_opt = opt_res$xmin
    pos_opt = optimize_all_time_lags_to_their_source_positions(
      lags = t(t(sp$shared_peaks$delays) - offsets_opt),
      receptors = receptors,
      sound_velocity = sound_velocity
    )
...
position_to_gcc_phase_matrix <-
  function(positions_matrix,
           receptors,
           receptor_shifts,
           gcc_phase_data,
           n_lags,
  ...
  {
    n_receptors = ncol(receptors)
    n_positions = ncol(positions_matrix)

    n_lags

    expected_delays <- vapply(X = 1:n_positions, function(i_pos)
      colSums((positions_matrix[, i_pos] - receptors) ^ 2) ^ 0.5 / sound_velocity,
      numeric(n_receptors))

    d = expected_delays - receptor_shifts
    lags = c(0:(nlags - 1), -nlags:-1)
    C_matrix <-
      vapply(X = 1:n_positions, function(i_pos)
        vapply(1:(n_receptors - 1), function(i_rec)
          vapply(i_rec:n_receptors, function(j_rec)
          {
            lag_ij = (d[i_pos, j_rec] - d[i_pos, i_rec]) * fs
            A = 1 / d[i_pos, j_rec] / d[i_pos, i_rec]
            return (ifelse((lags > lag_ij - 1) &
                             (lags < lag_ij + 1), (1 - abs(lags - lag_ij)) * A, 0))
          }, numeric(2 * nlags)),
          numeric(2 * nlags * (n_receptors - i_rec))),
        numeric(nlags * (n_receptors * (n_receptors - 1))))
    return (C_matrix)
  }

to_gcc_phase_vector <- function(receptors, gcc_phase_data, n_lags)
{
  n_receptors = ncol(receptors)

  nsamples = length(gcc_phase_data[[1]][[1]])
  i_lags = c(1:nlags, (1 - nlags):0 + nsamples)
  Y_vector <-
    Reduce(function(p, i_rec)
      c(p, vapply(i_rec:n_receptors, function(j_rec)
      {
        return (gcc_phase_data[[i_rec]][[j_rec - i_rec]][i_lags])
      }, numeric(2 * nlags))), 1:(n_receptors - 1), c())
  return (Y_vector)
