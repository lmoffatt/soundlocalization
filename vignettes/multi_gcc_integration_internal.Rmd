...
```{r}
ggplot2::ggplot(r_source$gcc_data_for_source_plot) + ggplot2::geom_raster(ggplot2::aes(x = lag_i_j, y = lag_i_k, fill = lag_values)) + ggplot2::geom_point(ggplot2::aes(x= lag_i_j, y = lag_i_k, size = source_nvalues_AB),color='red',alpha=0.2)+ ggplot2::geom_point(ggplot2::aes(x= lag_i_j, y = lag_i_k, size = source_nvalues_AC),color='green',alpha=0.2)+ ggplot2::geom_point(ggplot2::aes(x= lag_i_j, y = lag_i_k, size = source_nvalues_BC),color='yellow',alpha=0.2)+ facet_grid(i_k ~ i_j) + ggplot2::scale_fill_distiller(palette = "Spectral")
```

```{r}
plot_peaks_groups_by_Lasso(r_source)
```
...
```
```{r}
gphase_filter_peaks_data(gcc_data,lag_window_in_meters=8000,keep_the_best_n =1000,keep_if_z_is_greater_than = 100)->rp_500
plot_shared_peaks(rp_500)
